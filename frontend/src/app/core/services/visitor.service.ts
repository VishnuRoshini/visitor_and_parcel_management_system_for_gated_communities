...
import { Record, ApiResponse, PaginatedResponse, CreateVisitorRequest, VisitorStatus } from '../models';
...
export class VisitorService {
  ...
  create(data: CreateVisitorRequest): Observable<Record> {
  ...
  updateStatus(id: number, status: VisitorStatus): Observable<Record> {
  ...
  getAll(page: number = 1, limit: number = 20, status?: VisitorStatus): Observable<PaginatedResponse<Record>> {
  ...
  getMyVisitors(page: number = 1, limit: number = 20): Observable<PaginatedResponse<Record>> {
  ...
  getMyPendingVisitors(): Observable<Record[]> {
